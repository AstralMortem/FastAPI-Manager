...
from fastapi_manager.db.aerich.migrate import MIGRATE_TEMPLATE, Migrate
from fastapi_manager.db.aerich.models import Aerich
...
if TYPE_CHECKING:
    from fastapi_manager.db.aerich.inspectdb import Inspect  # noqa:F401
...
class Command:
    ...
    async def upgrade(self, run_in_transaction: bool = True) -> List[str]:
        ...
        for version_file in Migrate.get_all_version_files():
            try:
                exists = await Aerich.exists(
                    version=version_file, app=self.app or self.default_app
                )
            ...
            if not exists:
                ...
                if run_in_transaction:
                    ...
                    app_conn = get_app_connection(
                        self.tortoise_config, self.app or self.default_app
                    )
