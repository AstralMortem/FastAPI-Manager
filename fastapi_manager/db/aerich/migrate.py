...
from fastapi_manager.db.aerich.models import MAX_VERSION_LENGTH, Aerich
from fastapi_manager.db.aerich.utils import (
    get_app_connection,
    get_models_describe,
    is_default_function,
)
...
class Migrate:
    ...
    def _get_model(cls, model: str, app: str) -> Type[Model]:
        return Tortoise.apps[app][model]
    ...
    async def load_ddl_class(cls) -> Type[BaseDDL]:
        ddl_dialect_module = importlib.import_module(
            f"fastapi_manager.db.aerich.ddl.{cls.dialect}"
        )
    ...
    async def generate_version(cls, name=None) -> str:
        ...
        if len(version) > MAX_VERSION_LENGTH:
            raise ValueError(
                f"Version name exceeds maximum length ({MAX_VERSION_LENGTH})"
            )
        return version
    ...
    @classmethod
    def _handle_indexes(
        cls, model: Type[Model], indexes: List[Union[Tuple[str], Index]]
    ) -> list:
        ret: list = []
    ...
    @classmethod
    def _get_indexes(
        cls, model, model_describe: dict
    ) -> Set[Union[Index, Tuple[str, ...]]]:
    ...
    def diff_models(
        ...
        aerich_app = next(
            key for key, val in Tortoise.apps.items() if cls._aerich in val
        )
        _aerich = (
            f"{cls.app}.{cls._aerich}" if cls.app else f"{aerich_app}.{cls._aerich}"
        )
        ...
        for new_model_str, new_model_describe in new_models.items():
            ...
            model = cls._get_model(
                new_model_describe.get("name").split(".")[1],
                new_model_describe.get("app"),
            )
            ...
            if new_model_str not in old_models:
                ...
                if new_table != old_table:
                    cls._add_operator(
                        cls.rename_table(model, old_table, new_table), upgrade
                    )
                ...
                new_unique_together = set(
                    map(
                        ...
                        cast(
                            List[Iterable[str]],
                            new_model_describe.get("unique_together"),
                        ),
                    )
                )
                ...
                for action, option, change in diff(old_m2m_fields, new_m2m_fields):
                    ...
                    if action == "add":
                        ...
                        if add:
                            ref_desc = cast(
                                dict, new_models.get(new_value.get("model_name"))
                            )
                            cls._add_operator(
                ...
                for index in old_unique_together.difference(new_unique_together):
                    cls._add_operator(
                        cls._drop_index(model, index, True), upgrade, True
                    )
                # add indexes
                ...
                old_data_fields_name = cast(
                    List[str], [i.get("name") for i in old_data_fields]
                )
                new_data_fields_name = cast(
                    List[str], [i.get("name") for i in new_data_fields]
                )
                ...
                for new_data_field_name in set(new_data_fields_name).difference(
                    ...
                    new_data_field = cls.get_field_by_name(
                        new_data_field_name, new_data_fields
                    )
                    is_rename = False
                ...
                for old_data_field_name in set(old_data_fields_name).difference(
                    ...
                    if (upgrade and old_data_field_name in cls._rename_old) or (
                        ...
                        continue
                    old_data_field = cls.get_field_by_name(
                        old_data_field_name, old_data_fields
                    )
                ...
                old_fk_fields_name: List[str] = [
                    i.get("name", "") for i in old_fk_fields
                ]
                new_fk_fields_name: List[str] = [
                    i.get("name", "") for i in new_fk_fields
                ]
                ...
                for old_fk_field_name in set(old_fk_fields_name).difference(
                    ...
                    if old_fk_field.get("db_constraint"):
                        ref_describe = cast(
                            dict, old_models[old_fk_field["python_type"]]
                        )
                        cls._add_operator(
                ...
                # change fields
                for field_name in set(new_data_fields_name).intersection(
                    set(old_data_fields_name)
                ):
                    ...
                    for change in changes:
                        ...
                        if option == "indexed":
                            ...
                            if not (
                                is_default_function(old_new[0])
                                or is_default_function(old_new[1])
                            ):
                            ...
                            # change nullable
                            cls._add_operator(
                                cls._alter_null(model, new_data_field), upgrade
                            )
                        elif option == "description":
                            # change comment
                            cls._add_operator(
                                cls._set_comment(model, new_data_field), upgrade
                            )
                        else:
                            if modified:
                                continue
    ...
    @classmethod
    def rename_table(
        cls, model: Type[Model], old_table_name: str, new_table_name: str
    ) -> str:
    ...
    @classmethod
    def _resolve_fk_fields_name(
        cls, model: Type[Model], fields_name: Iterable[str]
    ) -> List[str]:
        ret = []
    ...
    @classmethod
    def _add_field(
        cls, model: Type[Model], field_describe: dict, is_pk: bool = False
    ) -> str:
    ...
    @classmethod
    def _rename_field(
        cls, model: Type[Model], old_field_name: str, new_field_name: str
    ) -> str:
    ...
    def _merge_operators(cls) -> None:
        ...
        for _upgrade_fk_m2m_operator in cls._upgrade_fk_m2m_index_operators:
            if (
                "ADD" in _upgrade_fk_m2m_operator
                or "CREATE" in _upgrade_fk_m2m_operator
            ):
        ...
        for _downgrade_fk_m2m_operator in cls._downgrade_fk_m2m_index_operators:
            if (
                "ADD" in _downgrade_fk_m2m_operator
                or "CREATE" in _downgrade_fk_m2m_operator
            ):
