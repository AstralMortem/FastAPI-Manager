...
from fastapi_manager.db.aerich.utils import is_default_function
...
class BaseDDL:
    ...
    _ALTER_DEFAULT_TEMPLATE = (
        'ALTER TABLE "{table_name}" ALTER COLUMN "{column}" {default}'
    )
    _RENAME_COLUMN_TEMPLATE = 'ALTER TABLE "{table_name}" RENAME COLUMN "{old_column_name}" TO "{new_column_name}"'
    _ADD_INDEX_TEMPLATE = (
    ...
    _CHANGE_COLUMN_TEMPLATE = 'ALTER TABLE "{table_name}" CHANGE {old_column_name} {new_column_name} {new_column_type}'
    _RENAME_TABLE_TEMPLATE = (
    ...
    def create_table(self, model: "Type[Model]") -> str:
        return self.schema_generator._get_table_sql(model, True)[
    ...
    def create_m2m(
        ...
        return self._M2M_TABLE_TEMPLATE.format(
            ...
            comment=(
                self.schema_generator._table_comment_generator(
    ...
    def add_column(
        self, model: "Type[Model]", field_describe: dict, is_pk: bool = False
    ) -> str:
        return self._add_or_modify_column(model, field_describe, is_pk)

    def _add_or_modify_column(
        self, model, field_describe: dict, is_pk: bool, modify=False
    ) -> str:
        db_table = model._meta.db_table
        description = field_describe.get("description")
        db_column = cast(str, field_describe.get("db_column"))
    ...
    def add_index(
        self, model: "Type[Model]", field_names: List[str], unique=False
    ) -> str:
    ...
    def add_fk(
        ...
        return self._ADD_FK_TEMPLATE.format(
            ...
            fk_name=self._generate_fk_name(
                db_table, field_describe, reference_table_describe
            ),
            db_column=db_column,
    ...
    def rename_table(
        self, model: "Type[Model]", old_table_name: str, new_table_name: str
    ) -> str:
        db_table = model._meta.db_table
